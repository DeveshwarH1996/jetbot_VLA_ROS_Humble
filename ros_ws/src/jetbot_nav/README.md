...
Controller: `nav2_regulated_pure_pursuit_controller` — simpler to tune than DWB/MPPI, a reasonable default for a small differential-drive robot with no manipulator.

**Footprint and velocity limits are not hardcoded here** — the values checked into `nav2_params.yaml` are placeholders only (marked `OVERRIDDEN AT LAUNCH` in comments); `launch/nav.launch.py`'s `_merge_nav2_params` overwrites them at launch time with the real values from `jetbot_base`'s `config/robot_params.yaml`, the single shared source of truth also used by `motor_driver` and `joy_controller`. Nav2's own launch API only accepts one `params_file`, so this reads both YAMLs, overwrites the relevant nested keys (costmap `footprint`, `FollowPath`'s velocity fields, `velocity_smoother`'s limits), and writes a combined temp file to actually launch with — verified by deliberately changing a shared value and confirming it propagated through to the merged file (`0.75 × max_linear_vel` showing up correctly in `desired_linear_vel`), not just trusting that the code looked right.
...
**Bugs found by actually launching this and checking `ros2 param list` against the real running nodes, not just reading Nav2 docs or assuming a written config took effect:**
1. `nav2_navfn_planner::NavfnPlanner` and `nav2_behaviors::Spin`/`BackUp`/`Wait` (C++-namespace plugin names) don't work — pluginlib only has these two packages registered under their `package/ClassName` form (`nav2_navfn_planner/NavfnPlanner`, `nav2_behaviors/Spin`, etc.), even though several *other* Nav2 plugins (the controller, costmap layers) accept both forms. `planner_server`/`behavior_server` fail to configure with a `FATAL` error naming the exact valid declared types.
2. `nav2_bringup`'s `navigation_launch.py` unconditionally starts a `smoother_server` node alongside the others, which this config's `lifecycle_manager.node_names` list initially omitted — it stayed configured but never activated until added.
3. **`FollowPath.max_angular_vel` isn't a real parameter of `nav2_regulated_pure_pursuit_controller` in Humble** — it was silently ignored the entire time (unknown keys in a params YAML don't error, they just never get declared/used by the node). Caught by running `ros2 param list /controller_server` against the live node and noticing it wasn't there. The actual angular-rate ceiling is enforced downstream by `velocity_smoother`'s `max_velocity` (which *is* real and confirmed working); the closest real `FollowPath` parameter, `rotate_to_heading_angular_vel`, only bounds the narrower "rotate in place to face the path" behavior, not general driving.
...
Also verified the `robot_params.yaml` merge specifically: `ros2 param get /jetbot_motor_driver wheel_base` and `/joy_controller max_linear_vel` both reflect the shared file after `bringup.launch.py`, and `ros2 param get /controller_server FollowPath.rotate_to_heading_angular_vel` / `/local_costmap/local_costmap footprint` / `/velocity_smoother max_velocity` all reflect the merged values on the actual running Nav2 nodes, not just in the generated temp file.
